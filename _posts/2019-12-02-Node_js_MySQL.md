...
在以下实例中根据你的实际配置修改数据库用户名、及密码及数据库：
...
| socketPath         | 连接到unix域路径，当使用host和port 时会被忽略                |
...
| dateStrings        | 强制timestamp，datatime,data；data类型以字符串类型返回，而不是JavaScript Data类型（默认：false) |
| debug              | 开启调试（默认：false)                                       |
...
将上面我们提供的SQL 文件导入数据库后，执行以上代码即可查询出数据：
